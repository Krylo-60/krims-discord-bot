...
client.once('ready', async () => {
  ...
  console.log(`[+] Cleaning existing non-managed roles in ${guild.name}...`);
  ...
  console.log(`[+] Building 100% perfectly ordered roles from bottom to top...`);
  for (const r of BOTTOM_TO_TOP_ROLES) {
    const created = await guild.roles.create({
      name: r.name,
      color: r.color,
      hoist: r.hoist,
      reason: 'Perfect hierarchy rebuild'
    }).catch(e => console.log('Err:', e.message));
    if (created) {
      console.log(`   [+] Created at Top: ${created.name}`);
    }
  }
  ...
  console.log('\n=== FINAL 100% PERFECT ROLE HIERARCHY ===');
  sorted.forEach((r, idx) => {
    console.log(`${idx + 1}. ${r.name} (Position ${r.position})`);
  });
