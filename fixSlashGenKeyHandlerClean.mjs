...
const brokenBlockStart = "if (commandName === 'genkey') {";
const brokenBlockEnd = "if (commandName === 'coinflip')";

const startIdx = code.indexOf(brokenBlockStart);
const endIdx = code.indexOf(brokenBlockEnd);

if (startIdx !== -1 && endIdx !== -1) {
  const cleanHandler = `if (commandName === 'genkey') {
  ...
  code = code.substring(0, startIdx) + cleanHandler + code.substring(endIdx);
  fs.writeFileSync('index.js', code);
  console.log('✅ Cleaned /genkey slash command handler syntax!');
} else {
  console.error('[-] Could not locate broken block in index.js');
}
